...
from utils.mesh_utils import clean_mesh, decimate_mesh, Mesh
...
class GaussianModel:
    ...
    @torch.no_grad()
    def extract_fields(self, resolution=128, num_blocks=64, relax_ratio=1.5):
        ...
        mask = (opacities > 0.0001).squeeze(1)
    ...
    def extract_mesh(self, path, density_thresh=0.001, resolution=128, decimate_target=1e5):

        os.makedirs(os.path.dirname(path), exist_ok=True)

        occ = self.extract_fields(resolution).detach().cpu().numpy()
        print(occ.min(), occ.max(), occ.mean())
        import mcubes
